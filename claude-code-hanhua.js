...
content = content
  ...
  .replaceAll(`"Connecting…"`, `"正在连接…"`)
  .replaceAll(`"Creating worktree…"`, `"正在创建工作树…"`)
  .replaceAll(`"Loading context usage…"`, `"正在加载上下文用量…"`)
  .replaceAll(`"Loading sessions…"`, `"正在加载会话…"`)
  .replaceAll(`"Loading..."`, `"加载中…"`)
  .replaceAll(`"Signing in…"`, `"正在登录…"`)
  .replaceAll(`"Waiting for permission…"`, `"等待权限…"`)
  // 操作/按钮
  .replaceAll(`"Delete session"`, `"删除会话"`)
  .replaceAll(`"Reconnect"`, `"重新连接"`)
  .replaceAll(`"Rewind to…"`, `"回滚至…"`)
  .replaceAll(`"Remove attachment"`, `"移除附件"`)
  .replaceAll(`"Remove comment"`, `"移除批注"`)
  .replaceAll(`"Copy Link"`, `"复制链接"`)
  .replaceAll(`"Copy code"`, `"复制代码"`)
  .replaceAll(`"Copy code to clipboard"`, `"复制代码到剪贴板"`)
  .replaceAll(`"Close banner"`, `"关闭横幅"`)
  .replaceAll(`"Dismiss error"`, `"关闭错误"`)
  .replaceAll(`"Dismiss survey"`, `"关闭调查"`)
  .replaceAll(`"Skip"`, `"跳过"`)
  .replaceAll(`"Create"`, `"创建"`)
  .replaceAll(`"Close"`, `"关闭"`)
  .replaceAll(`"Back"`, `"返回"`)
  .replaceAll(`"Search"`, `"搜索"`)
  // 标签/标题
  .replaceAll(`"Explored"`, `"已探索"`)
  .replaceAll(`"browser tab"`, `"浏览器标签页"`)
  .replaceAll(`"Bad response"`, `"差评"`)
  .replaceAll(`"Good response"`, `"好评"`)
  .replaceAll(`"Message input"`, `"消息输入"`)
  .replaceAll(`"Usage window"`, `"用量窗口"`)
  .replaceAll(`"Feedback"`, `"反馈"`)
  .replaceAll(`"No questions provided"`, `"未提供问题"`)
  .replaceAll(`"has not changed"`, `"未发生更改"`)
  .replaceAll(`"read-only"`, `"只读"`)
  .replaceAll(`"% of usage"`, `"用量占比"`)
  .replaceAll(`"Agent:"`, `"智能体："`)
  // 工具名称
  .replaceAll(`"Web Fetch"`, `"网页抓取"`)
  .replaceAll(`"Web Search"`, `"网页搜索"`)
  .replaceAll(`"Artifact"`, `"构件"`)
  .replaceAll(`"Glob"`, `"通配符搜索"`)
  .replaceAll(`"Grep"`, `"内容搜索"`)
  .replaceAll(`"REPL"`, `"交互执行"`)
  // 其他
  .replaceAll(`"Troubleshooting resources"`, `"故障排除资源"`)
  .replaceAll(`"Learn how we use it"`, `"了解我们如何使用它"`)
  .replaceAll(`"Can't switch branches"`, `"无法切换分支"`)
  .replaceAll(`"Claude's Plan"`, `"Claude 的计划"`)
  .replaceAll(`"Day"`, `"日"`)
  .replaceAll(`"Week"`, `"周"`)
  .replace(`"Approximate, based on local sessions on this machine — does not include other devices or claude.ai"`, `"基于本机本地会话的近似值 — 不包括其他设备或 claude.ai"`)
  .replace(`"What's contributing to your limits usage?"`, `"什么在消耗您的用量限额？"`)
